...
def interpret_ph(ph: float) -> str:
    ...
    if ph < 5.0:
        ...
        return "Alcalino. Condiciones desfavorables, puede favorecer condiciones asociadas con mayor riesgo de sarna común en la papa."
...
def interpret_texture(sand: float, silt: float, clay: float) -> str:
    ...
    Nota: Esta es una heurística simplificada del proyecto AgroField AI y 
    no constituye una clasificación formal completa mediante triángulo textural.
    """
    if sand > 65:
...
def run(soil_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(ph_data, dict) and "valor" in ph_data:
    ...
    if isinstance(soc_data, dict) and "valor" in soc_data:
    ...
    if isinstance(sand_data, dict) and isinstance(silt_data, dict) and isinstance(clay_data, dict):
        if "valor" in sand_data and "valor" in silt_data and "valor" in clay_data:
            sand_val = sand_data["valor"]
            silt_val = silt_data["valor"]
            clay_val = clay_data["valor"]
            
            # Verificar si suman aproximadamente 100%
            total = sand_val + silt_val + clay_val
            if not (90 <= total <= 110):
                agronomic_notes.append(f"Advertencia: La suma de textura ({total:.1f}%) difiere significativamente del 100%. Las unidades podrían necesitar revisión.")
